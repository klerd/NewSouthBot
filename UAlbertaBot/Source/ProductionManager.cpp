...
{
	//BuildOrder bo = StrategyManager::Instance().predict();
	//setBuildOrder(StrategyManager::Instance().getOpeningBookBuildOrder());
	//setBuildOrder(StrategyManager::Instance().predict());
	if (Config::Strategy::StrategyName == "NewSouthBot") {
		setBuildOrder(StrategyManager::Instance().selectOpeningStrategy());
	}
	else {
		setBuildOrder(StrategyManager::Instance().getOpeningBookBuildOrder());
	}
	//ProductionManager::Instance().performBuildOrderSearch();
}
...
void ProductionManager::setBuildOrder(const BuildOrder & buildOrder)
{
	...
	{
		//_queue.queueAsLowestPriority(BWAPI::UnitTypes::Protoss_Pylon, true);
...
{	
	...
	//Commented out original strategy, Using Prediction strategy:
